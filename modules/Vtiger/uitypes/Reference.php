...
class Vtiger_Reference_UIType extends Vtiger_Base_UIType
{
	...
	public function getDisplayValue($value, $record = false, $recordInstance = false, $rawText = false)
	{
		...
		if ($referenceModule && !empty($value)) {
			...
			if($referenceModuleName === 'Documents')
			{
				$documentRecordModel = Documents_Record_Model::getInstanceById($value);

				$linkValue ="<a class='btn btn-default btn-sm popoverTooltip' href='".$documentRecordModel->getDownloadFileURL()."' data-docfilter='1' data-content='Apri Documento'><span class='glyphicon glyphicon-open-file' aria-hidden='true'></span> Apri</a>&nbsp;&nbsp;";
	...
	public function getListViewDisplayValue($value, $record = false, $recordInstance = false, $rawText = false)
	{
		...
		if ($referenceModule && !empty($value)) {
			...
			$linkValue ="<a class='btn btn-default btn-sm' style='float:left;' role='button' href='index.php?module={$referenceModuleName}&view={$referenceModule->getDetailViewName()}&record={$value}' title='{$title}'><span class='glyphicon glyphicon-link' aria-hidden='true'></span></a>&nbsp;&nbsp;<span class='moduleColor_{$referenceModuleName}'>{$name}</span>";
			return $linkValue;
		}
		return '';
	}
